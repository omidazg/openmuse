...
test("supported documents pass; images, other types and oversized files get Persian errors", () => {
  assert.equal(
  ...
  assert.match(uploadProblem({ name: "image.png", size: 10, type: "image/png" }) ?? "", /تصویر/);
  assert.match(
